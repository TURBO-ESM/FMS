   ...
   if (verbose)
     {
       ...
       printf ("For estimating RSS for a different value of k, multiply above "
               "usage by k.\n");
     }
   else
...
char** find_partitioned_files(const char* filepath, int* count) {
  ...
  *count = (int)globbuf.gl_pathc;
  ...
  // Allocate array of file-path strings
  ...
  // Copy each matched path into our array
 ...
 main_ (int argc, char *argv[])
 {
   ...
   int bf = DEFAULT_BF; /* default blocking factor: bf records will be read and
                           written at a time */
   int nblocks
       ...
       if (!strcmp (argv[a], "-v"))
       ...
       else if (!strcmp (argv[a], "-k"))
         {
           ...
           if (bf > MAX_BF)
             {
               fprintf (stderr,
                        "Warning: k is set to high. Choosing a more sane value "
                        "of %d.\n",
                        MAX_BF);
               bf = MAX_BF;
             }
   ...
   if (mem_dry_run)
     {
       if (bf != 1)
         fprintf (stderr, "-x is set, so blocking factor will be set to 1. The "
                          "-k option will be ignored.\n");
       bf = 1;
       if (verbose)
   ...
   if (!mem_dry_run)
     {
       ...
       if ((ncoutfile = (struct fileinfo *)malloc (sizeof (struct fileinfo)))
           == NULL)
         {
       ...
       if (!appendnc)
         {
           ...
           status
               = nc__create (outfilename, format, 0, &blksz, &ncoutfile->ncfid);
           if (status == (-1))
             {
               fprintf (stderr,
       ...
       /* Open an existing netCDF file for appending */
       else
         {
           if ((ncoutfile->ncfid = ncopen (outfilename, NC_WRITE)) == (-1))
             {
               fprintf (stderr, "Error: cannot open the output netCDF file for "
                                "appending!\n");
               free (ncoutfile);
               return (1);
             }
   ...
   if (inputarg == (-1))
     {
       int num_infiles_used
           = 0; /* we may use only a subset of the input files */
       if (nend > -1)
             ...
             for (a = nstart; a <= nend; a++)
               {
                 ...
                 if (stat (infilename, &statbuf) != 0)
                   {
                     if (force == 0)
                       {
                         printf ("ERROR: missing at least %s from the input "
                                 "file set.  Exiting.\n",
                                 infilename);
                         unlink (outfilename);
                         return 9;
                 ...
                 infileerror = process_file (
                     infilename, appendnc, ncoutfile, outfilename, &nfiles,
                     &nrecs, &nblocks, &bf, block, f, headerpad, verbose,
                     missing, deflate, deflation, shuffle);
                 if (infileerror)
                   infileerrors = 1;
                 appendnc = 1;
                 f++;
                 if (f == nfiles || a == nend)
                   {
                     ...
                     for (r = block * bf; r < min ((block + 1) * bf, nrecs);
                          r++)
                       outfileerrors
                           += flush_decomp (ncoutfile, nfiles, r, bf, verbose);
                     num_infiles_used = f;
                     break;
                   }
               }
                   ...
                   if (f == nfiles || a == (nend - 1))
                     {
                       ...
                       for (r = block * bf; r < min ((block + 1) * bf, nrecs);
                            r++)
                         outfileerrors += flush_decomp (ncoutfile, nfiles, r,
                                                        bf, verbose);
                       f = 0;
                       continue;
                     }
         ...
         for (a = inputarg; a < argc; a++)
           {
             ...
             if (stat (argv[a], &statbuf) != 0)
               {
                 if (force == 0)
                   {
                     printf ("ERROR: missing at least %s from the input file "
                             "set.  Exiting.\n",
                             argv[a]);
                     unlink (outfilename);
                     return 9;
             ...
             infileerror = process_file (argv[a], appendnc, ncoutfile,
                                         outfilename, &nfiles, &nrecs, &nblocks,
                                         &bf, block, f, headerpad, verbose,
                                         missing, deflate, deflation, shuffle);
             if (infileerror)
               infileerrors = 1;
             appendnc = 1;
               ...
               for (a = nstart; a <= nend; a++)
                 {
                   ...
                   // and yet the user wants to remove the input files in a
                   // range.
                   // if (++f > nfiles) break;
                   if (peWidth < 0)
                     {
 ...
 usage ()
 {
   printf ("mppnccombine %s - (written by Hans.Vahlenkamp@noaa.gov)\n\n",
           version);
   printf ("Usage:  mppnccombine [-v] [-V] [-M] [-a] [-r] [-n #] [-k #] [-e #] "
           "[-h #] [-64] [-n4 [-d #] [-s]] [-m]\n");
   ...
   printf ("  -r    Remove the \".####\" decomposed files after a successful "
           "run.\n");
   printf ("  -n #  Input filename extensions start with number #### instead "
           "of 0000.\n");
   printf ("  -k #  Blocking factor. k records are read from an input file at "
           "a time.\n");
   printf ("        Valid values are between 0 and %d. For a given input, the "
           "maximum\n",
           MAX_BF);
   printf (
       "        permissible value for k is min(total number of records, %d).\n",
       MAX_BF);
   printf ("        Setting k to zero will set the blocking factor to this "
           "maximum\n");
   printf ("        permissible value. Setting k to a value higher than this "
           "value,\n");
   printf ("        will make the system implicitly set k to the highest "
           "permissible value.\n");
   printf ("        A value of 1 for k disables blocking. This is the default "
           "behavior.\n");
   printf ("        Blocking often improves performance, but increases the "
           "peak memory\n");
   printf ("        footprint (by the blocking factor). Beware of running out "
           "of\n");
   printf ("        available physical memory and causing swapping to disk due "
           "to\n");
   ...
   printf ("  -e #  Ending number #### of a specified range of input filename "
           "extensions.\n");
   printf ("        Files within the range do not have to be consecutively "
           "numbered.\n");
   printf ("  -h #  Add a specified number of bytes of padding at the end of "
           "the header.\n");
   printf (
       "  -64   Create netCDF output files with the 64-bit offset format.\n");
   printf ("  -n4   Create netCDF output files in NETCDF4_CLASSIC mode (no v4 "
           "enhanced features).\n");
   printf ("  -d #  When in NETCDF4 mode, use deflation of level #.\n");
   printf ("  -s    When in NETCDF4 mode, use shuffle.\n");
   printf ("  -m    Initialize output variables with a \"missing_value\" from "
           "the variables\n");
   printf ("        of the first input file instead of the default 0 value.\n");
   printf ("  -x    Print an estimate for peak memory resident size in (MB) "
           "and exit.\n");
   printf ("        No output file will be created. Setting -x automatically "
           "sets\n");
   printf (
       "        the blocking factor (-k) to 1. Any value set for -k on the\n");
   printf ("        command-line will be ignored. To estimate memory usage for "
           "a\n");
   printf ("        a different blocking factor, simply multiply the estimate "
           "by k.\n\n");
   printf ("mppnccombine joins together an arbitrary number of netCDF input "
           "files, each\n");
   printf ("containing parts of a decomposed domain, into a unified netCDF "
           "output file.\n");
   printf ("An output file must be specified and it is assumed to be the first "
           "filename\n");
   printf ("argument.  If the output file already exists, then it will not be "
           "modified\n");
   printf ("unless the option is chosen to append to it.  If no input files "
           "are specified\n");
   printf ("then their names will be based on the name of the output file plus "
           "the default\n");
   printf ("numeric extension \".0000\", which will increment by 1.  There is "
           "an option for\n");
   printf ("starting the filename extensions with an arbitrary number instead "
           "of 0.  There\n");
   printf ("is an option for specifying an end to the range of filename "
           "extension numbers;\n");
   ...
   printf ("A value of 0 is returned if execution completed successfully; a "
           "value of 1\n");
   printf ("otherwise.\n");
 }
 ...
 int
 process_file (char *ncname, unsigned char appendnc, struct fileinfo *ncoutfile,
               char *outncname, int *nfiles, int *nrecs, int *nblocks, int *bf,
   ...
   if (ncattget (ncinfile->ncfid, NC_GLOBAL, "NumFilesInSet", (void *)&nfiles2)
       == (-1))
     {
       if (*nfiles == 1)
         {
           fprintf (stderr,
                    "Error: missing the \"NumFilesInSet\" global attribute!\n");
           return (1);
         }
       else if (*nfiles == (-1))
         {
           fprintf (
               stderr,
               "Warning: missing the \"NumFilesInSet\" global attribute.\n");
         }
     }
   *nfiles = nfiles2;
   ...
   for (v = 0; v < ncinfile->nvars; v++)
     {
       if ((ncvarinq (ncinfile->ncfid, v, ncinfile->varname[v],
                      ...
                      ncinfile->vardim[v], &(ncinfile->natts[v])))
           == (-1))
         {
   ...
   if ((!appendnc) && (!mem_dry_run))
     {
       ...
       for (v = 0; v < ncinfile->nvars; v++)
         {
           ncvardef (ncoutfile->ncfid, ncinfile->varname[v],
                     ncinfile->datatype[v], ncinfile->varndims[v],
                     ncinfile->vardim[v]);
           for (n = 0; n < ncinfile->natts[v]; n++)
             {
               ...
               if (!strcmp (attname, "domain_decomposition"))
                 ...
                 {
                   if (ncattcopy (ncinfile->ncfid, v, attname, ncoutfile->ncfid,
                                  v)
                       == (-1))
                     {
                       fprintf (
                           stderr,
                           "Error: cannot copy variable \"%s\"'s attributes!\n",
                           ncinfile->varname[v]);
                       free (ncinfile);
                       return (1);
                     }
       ...
       for (n = 0; n < ncinfile->ngatts; n++)
         {
           ...
           if (!strcmp (attname, "NumFilesInSet"))
             ...
             {
               if (ncattcopy (ncinfile->ncfid, NC_GLOBAL, attname,
                              ncoutfile->ncfid, NC_GLOBAL)
                   == (-1))
                 {
                   fprintf (
                       stderr,
       ...
       if (deflate == 1 && deflation > 0)
         {
           for (v = 0; v < ncinfile->nvars; v++)
             {
               if (nc_def_var_deflate (ncoutfile->ncfid, v, shuffle, deflate,
                                       deflation)
                   != NC_NOERR)
                 {
                   fprintf (stderr, "Error: nc_def_var_deflate failed! Are you "
                                    "setting NETCDF4?\n");
                   usage ();
                 }
             }
   ...
   do
     {
       tmp_mem_alloc
           = mem_allocated; /* store current memory usage in a temporary */
       ncinfileerror
           += process_vars (ncinfile, ncoutfile, appendnc, nrecs, nblocks, bf,
 ...
 int
 process_vars (struct fileinfo *ncinfile, struct fileinfo *ncoutfile,
               unsigned char appendnc, int *nrecs, int *nblocks, int *bf, int r,
               int nfiles, int f, unsigned char verbose, unsigned char missing)
 {
       ...
       if ((*bf) >= 1)
         {
           if ((*bf) > (*nrecs))
             {
               fprintf (stderr,
                        "blocking factor (k) > total records (%d). Setting "
                        "blocking factor to %d.\n",
                        *nrecs, *nrecs);
               *bf = *nrecs;
             }
         }
       else
         {
           ...
           *bf = min (
               MAX_BF,
               *nrecs); // we use the maximum blocking factor in our capacity
   ...
   /* Allocate memory for the decomposed variables, if none has been allocated
      ...
      malloc, so the entire allocation appears to be a double-dimensional array.
      The memory allocated below is a trivial amount. The real allocation will
      happen later. This allocation is done exactly once in the whole program */
   if (varbuf == NULL)
     {
       int nbytes
           = (*bf) * sizeof (void **) + ((*bf) * MAX_NC_VARS * sizeof (void *));
       if (verbose || print_mem_usage)
         fprintf (stderr,
                  "allocating a buffer of %d bytes for the multi-dimensional "
                  "pointer array\n",
                  nbytes);
       if (mem_dry_run)
       ...
       for (z = 0; z < (*bf); z++)
         {
           varbuf[z] = (void **)((size_t)varbuf + (*bf) * sizeof (void **)
                                 + z * MAX_NC_VARS * sizeof (void *));
         }
       /* The nested memory alloc works fine, but it involves multiple malloc
         calls. varbuf = (void ***)malloc((*bf) * sizeof(void **)); if (varbuf
         == NULL) { fprintf(stderr, "Could not allocate memory\n"); exit(1);
         }
         int z;
         for (z=0; z<(*bf); z++) {
   ...
   for (v = 0; v < ncinfile->nvars; v++)
     {
       ...
       if (r > 0)
         {
           ...
           if ((dimid = ncdimid (ncinfile->ncfid, ncinfile->varname[v]))
               != (-1))
             {
       ...
       if ((values = malloc (nctypelen (ncinfile->datatype[v]) * recsize))
         ...
         {
           fprintf (stderr,
                    "Error: cannot allocate %lld bytes for decomposed variable "
                    "\"%s\"'s values!\n",
       ...
       else
         {
           ...
           if (varbuf[(r % (*bf))][v] == NULL)
             {
               ...
               if (verbose && print_mem_usage)
                 printf ("      allocating %lld bytes for full domain of "
                         "variable %s\n",
               ...
               if ((varbuf[(r % (*bf))][v] = calloc (varbufsize, 1)) == NULL)
                 {
                   fprintf (stderr,
                            "Error: cannot allocate %lld bytes for entire "
                            "variable \"%s\"'s values!\n",
                            varbufsize, ncinfile->varname[v]);
                   return (1);
                 }
           ...
           imaxfull
               = ncinfile->dimfullsize[ncinfile->vardim[v][ncinfile->varndims[v]
                                                           - 1]];
           if (ncinfile->varndims[v] > 1)
             jmaxfull = ncinfile->dimfullsize
                            [ncinfile->vardim[v][ncinfile->varndims[v] - 2]];
           else
             jmaxfull = 1;
           if (ncinfile->varndims[v] > 2)
             kmaxfull = ncinfile->dimfullsize
                            [ncinfile->vardim[v][ncinfile->varndims[v] - 3]];
           else
             kmaxfull = 1;
           if (ncinfile->varndims[v] > 3)
             {
               if (ncinfile->vardim[v][ncinfile->varndims[v] - 4]
                   != ncinfile->recdim)
                 lmaxfull
                     = ncinfile->dimfullsize
                           [ncinfile->vardim[v][ncinfile->varndims[v] - 4]];
               else
                 lmaxfull = 1;
             }
           else
             lmaxfull = 1;
           if (verbose > 1)
             printf ("      imaxfull=%lld  jmaxfull=%lld  kmaxfull=%lld  "
                     "lmaxfull=%lld\n",
           ...
           if (verbose > 1)
             printf ("      ioffset=%lld  joffset=%lld  koffset=%lld  "
                     "loffset=%lld\n",
               ...
               for (l = 0; l < lmax; l++)
                 for (k = 0; k < kmax; k++)
                   for (j = 0; j < jmax; j++)
                     for (i = 0; i < imax; i++)
                       {
                         offset = (i + ioffset) + (j + joffset) * imaxfull
   ...
   for (v = 0; v < ncoutfile->nvars; v++)
     {
       ...
       if (ncvarput (ncoutfile->ncfid, v, outstart, count, varbuf[(r % bf)][v])
           == (-1))
         {
